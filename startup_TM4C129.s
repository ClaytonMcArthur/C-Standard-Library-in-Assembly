...
Reset_Handler   PROC
		...
		ISB     ; Let's leave as is from the original.
        	...
        	BLX     R0
                ...
                ; Step 2: initialize SVC syscall table
                LDR     R0, =_syscall_table_init
                BLX     R0

                ; Step 2: initialize your heap allocator
                LDR     R0, =_kinit
                BLX     R0
                ...
                ;switch the user stack for thread mode
                LDR     R0, =__initial_user_sp
                MSR     PSP, R0

                ; CONTROL use PSP, unprivileged thread
                MOVS    R0, #3
                MSR     CONTROL, R0
                ISB

                ; jump to C entry (__main)
                LDR     R0, =__main
                BX      R0
                ENDP
...
SVC_Handler     PROC
        ...
        ; pick the right stack frame pointer into R0
        TST     LR, #4
        ITE     EQ
        MRSEQ   R0, MSP       ; if bit-2==0, MSP was used
        MRSNE   R0, PSP       ; else PSP was used

        PUSH    {LR}         ; save EXC_RETURN
        BL      _systemcall_table_jump
        POP     {PC}         ; restore EXC_RETURN into PC ? proper exception return
        ENDP
...
SysTick_Handler PROC
                ...
                ; save any scratch regs you’ll use
                PUSH    {R0-R3, LR}
                ...
                ; force exception return to thread mode on PSP
                LDR     LR, =0xFFFFFFFD
                BX      LR
                ENDP
